...
function scrape (url, cache, logger, callback, options) {
  ...
  var domDomain     = domain.create(),
      data          = {links:[]}; 


  domDomain.on('error', function(err) {
    var u = require('url').parse(url);
    data["favicon"] = u.protocol + "//" + u.host + "/favicon.ico";
    callback(err, data);
  });


  domDomain.run(function () {
      ...
      if(url){
        ...
        request({uri: url}, function(requestErrors, response, body){
          if(!requestErrors && response.statusCode === 200){
            ...
            logger.warn('http error: '  + response.statusCode + ' (' + httpCodes[response.statusCode] + ') - ' + url);
            callback({'error': 'http status: ' + response.statusCode + ' (' + httpCodes[response.statusCode] + ') - ' + url}, null);
          }    
        });
      }else{
        // add error information
        logger.warn('no url given');
        callback({'error':'no url given'}, null);
      }
    }
