...
Grapher.prototype = {
  ...
  verifyPages: function () {
    ...
    _.each(this.pages, function (page) {
      ...
      if (!page.verified) {
        ...
        if (page.verified) verifiedStuff = true;
      }

    });

    if (verifiedStuff) self.verifyPages();
  },
  ...
  toJSON: function () {
    ...
    if (self.options.strict) {
      ...
      _.each(self.pages, function (page) {
        if (page.verified) results.push(page.toLiteral(['url', 'title', 'favicon', 'links']));
      });

    } else {

      results = _.map(self.pages, function (page) {
        return page.toLiteral(['url', 'title', 'favicon', 'links', 'verified']);
      });
