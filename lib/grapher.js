...
Grapher.prototype = {
  ...
  // Returns and array of error objects for any/all errors
  // encountered during the construction of the graph
  getErrors: function () {
    var rtnArr = [];

    _.each(this.pages, function (page) {
      if (page.errors.length > 0) rtnArr.push({
        url: page.url,
        errors: _.pluck(page.errors, 'error')
      });
    });

    return rtnArr.length > 0 ? rtnArr : undefined;
  },
...
function graph (url, options, callback) {
  ...
  grapher.build(function (graph) {
    ...
    if (callback) {
      callback(graph.getErrors(), graph.toLiteral());
    }

    deferred.resolve(graph.getErrors(), graph.toLiteral());
  });
